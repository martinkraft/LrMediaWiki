...
LrFunctionContext.callWithContext( 'dialogExample', function( context )
    ...
    local contents = f:view { 
        ...
        f:row { 
            margin_bottom = 10,
            f:static_text {
                title = "Generate filename und description for ".. ((#photos == 1) and "one photo" or (#photos .. " photos") ).." from face recognition data.",
            },
        },
        f:separator {
            fill_horizontal = 1
        ...
        local data = LrTasks.startAsyncTask(function()
            ...
            for key,photo in pairs(photos) do 
                ...
                --catalog:withPrivateWriteAccessDo('Set Filename', function()
                    ...
                    if props.title then 
                        photo:setRawMetadata( 'title', string.gsub( string.gsub(props.title, '{{f}}', fname.preName), '{{p}}', u.getNames(regions, {
                    ...
                    if props.description_en then 
                        ...
                        des = string.gsub(props.description_en, '{{p}}', u.getNames(regions, {
                            last = " and ",
                            inter = ", ",
                        }) )
                    ...
                    if props.categories then 
                        local des = string.gsub(props.categories, '{{p}}', u.getNames(regions, {
                            inter = "",
                            after = ";",
                        }) )
                    ...
                    if regions[2] then
                        photo:setPropertyForPlugin( _PLUGIN, 'otherFields', u.getImageNotes(regions, photo) )
                    end
